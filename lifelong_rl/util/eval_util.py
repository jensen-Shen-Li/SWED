...
def get_generic_path_information(paths, eval_env=None, stat_prefix=''):
    """
    ...
    rewards_list = []
    for i in range (10):
        returns = [sum(path["rewards"]) for path in paths[i]]
        returns_list.append(returns)
    for i in range (10):
        rewards = np.vstack([path["rewards"] for path in paths[i]]) 
        rewards_list.append(rewards)
    rewards = np.array(rewards_list).reshape(-1, 1)

    normalized_score = (eval_env.get_normalized_score(np.array(returns_list)) * 100.0).mean()
    ...
    statistics[stat_prefix + 'Normalized Scores'] = normalized_score
    '''
...
def get_average_returns(paths):
    ...
    return_list=np.squeeze(return_list)
